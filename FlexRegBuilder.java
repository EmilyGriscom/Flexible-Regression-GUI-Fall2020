...
public class FlexRegBuilder extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
            ...
            if(response == 0) { // picked one file																					// if user picked "One File"
            	...
            	if (tmp == JFileChooser.APPROVE_OPTION) {
                    if(ButtonActions.isXML(file.getSelectedFile().getAbsolutePath())) {							// If it's an XML file
                        ...
                        // new code by Emily Griscom
                        // add blank bean to array for this new file
                        beans.add(null);
                        
                        errorLabel.setVisible(false);
                    } else {
                        errorLabel.setText("Error: Not an .xml file");
                    	errorLabel.setVisible(true);
                    }
            	}
            	 
             } else if (response == 1) { // picked a folder																			// if user picked "Folder"
			...
			try {
                                // new code by Emily Griscom
                                // delete file's bean from array
                                beans.remove(rightTable.getSelectedRow());
